...
class SignPlacementRandomizer:
    def __init__(self):
        ...
        self.sign_left_origin = [-6, 0, 0]
        self.sign_location_offset_range = {"min":0,"max":7}
        self.sign_hight_range = {"min":150,"max":250}
    ...
    def sign_right_location_assign(self):
        """ 
        """
        sign_loc_list = []
    ...
    def sign_left_location_assign(self):
        """ 
        """
        sign_loc_list = []
        current_sign_loc_y = 0
    ...
    def import_sign_asset(self):
        ...
        ## randomly select k(k=num_sign_need_import) sign asset from sign_asset_path_list, then import to SignCollection
        sign_asset_path_list_selected = random.choices(sign_asset_path_list, k = self.num_sign_need_import)
    ...
    def sign_placement_randomize(self):
        """ 
        """
        sign_loc_list = np.add(self.sign_right_location_assign(), self.sign_right_origin).tolist() + \
                        np.add(self.sign_left_location_assign(), self.sign_left_origin).tolist()
